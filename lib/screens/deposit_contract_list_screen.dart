...
import 'deposit_contract_detail_screen.dart';
...
class DepositContractListScreen extends StatefulWidget {
  ...
  @override
  State<DepositContractListScreen> createState() =>
      _DepositContractListScreenState();
}

class _DepositContractListScreenState extends State<DepositContractListScreen> {
  ...
  void initState() {
    super.initState();
    _listFuture = widget.depositService.getMyDeposits();
  }
  ...
  Future<void> _refresh() async {
    final future = widget.depositService.getMyDeposits();
    setState(() {
      _listFuture = future;
    });
    await future;
            ...
            child: Column(
              children: [
                ...
                _FilterBar(
                  ...
                  statusOptions: _depositStatusOptions,
                ),
                Expanded(
  ...
  void _openDetail(ContractListItem item) {
    Navigator.of(context).push(
      MaterialPageRoute(
        builder: (context) => DepositContractDetailScreen(depositId: item.id),
      ),
    );
  }
  ...
  String _errorMessage(Object? error) {
    ...
    return 'Không tải được danh sách HĐ cọc';
  }
}

// ── Filter Bar ──

const _depositStatusOptions = <String, String>{
...
class _DepositCard extends StatelessWidget {
  const _DepositCard({required this.item, required this.onTap});
